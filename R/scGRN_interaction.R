...
scGRN_interaction = function(hic_interaction, enhancers, ref_promoters = 'all',up_stream = 2500,
  ...
  ehs <- GRanges(seqnames = enhancers$chr, ranges = IRanges(start = enhancers$start,
                                                              end = enhancers$end))
  ...
  if(ncol(hic_interaction)==6){
    anchor.one <- GRanges(hic_interaction$chr1,
                          IRanges(start = hic_interaction$start1, end = hic_interaction$end1))
    anchor.two <- GRanges(hic_interaction$chr2,
                          IRanges(start = hic_interaction$start2, end = hic_interaction$end2))
    hic <- GenomicInteractions(anchor.one, anchor.two)

  }else{
    anchor.one <- GRanges(hic_interaction$chr,
                          IRanges(start = hic_interaction$start1, end = hic_interaction$end1))
    anchor.two <- GRanges(hic_interaction$chr,
                          IRanges(start = hic_interaction$start2, end = hic_interaction$end2))
    hic <- GenomicInteractions(anchor.one, anchor.two)
  }
  ...
  hg19.transcripts <- transcriptsBy(txdb, by="gene")
  hg19.transcripts <- hg19.transcripts[ names(hg19.transcripts)  %in% unlist(hg19.genes$gene_id) ]
  hg19_refseq_promoters <- promoters(hg19.transcripts,upstream = up_stream,
                                     downstream = down_stream)

  seqnames_record <- seqnames(hg19_refseq_promoters)
  hg19_refseq_promoters <- unlist(hg19_refseq_promoters[seqnames_record %in%
                                                          c('chrX','chrY',paste('chr',1:22,sep=''))])
  hg19_refseq_promoters <- unique(hg19_refseq_promoters)
  gene_names <- getBM(attributes = c("hgnc_symbol",'entrezgene_id','ensembl_gene_id'),
  ...
  if(typeof(ref_promoters) == typeof('all')){
    ...
    ref_promoters_granges <- GRanges(
                             seqnames = ref_promoters$chr,
                             ranges = IRanges(start = ref_promoters$start, end = ref_promoters$end))
    overlap_granges <- findOverlapPairs(ref_promoters_granges,hg19_refseq_promoters,
   ...
   interaction_index <- isInteractionType(hic, "promoter", "enhancer")
   hic_subset <- hic[interaction_index]

   if(ncol(hic_interaction) == 6){
     df1 <- data.table(promoters = mcols(anchorOne(hic_subset))$promoter.id,
                       promoter_chr = hic_interaction$chr1[interaction_index],
                       enhs = mcols(anchorTwo(hic_subset))$enhancer.id,
                       enh_chr = hic_interaction$chr2[interaction_index])
     df1 <- df1[ (!is.na(df1$promoters)) & (!is.na(df1$enhs)) ,]

     df2 <- data.table(promoters = mcols(anchorTwo(hic_subset))$promoter.id,
                       promoter_chr = hic_interaction$chr2[interaction_index],
                       enhs = mcols(anchorOne(hic_subset))$enhancer.id,
  ...
  df <- left_join(df,df_enhancers,by = 'id')[,c('promoters','promoter_chr','enhancer','enh_chr')]
  df <- data.table(df)
  df$id <- seq.int(nrow(df))
  df_promoters <- df[,c('promoters','id')][, .(promoter = unlist(promoters)),by = id]
  df <- left_join(df,df_promoters,by = 'id')[,c('promoter','promoter_chr','enhancer','enh_chr')]
  df <- distinct(df)
  sp_list  <- tstrsplit(df$enhancer,"_|:|-", keep = c(2,3,4))
  ...
  final_df <- full_join(ref_df,df,by = 'promoter')
